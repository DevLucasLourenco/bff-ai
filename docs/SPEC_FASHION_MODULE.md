...
Adicionar tool exige: schema versionado, executor, autorização, teste de contrato e documentação. Adicionar objeto exige schema backend, tipo TS, renderer e fallback para versões antigas. O backend é dono do contrato; a LLM não injeta JSX, CSS ou JSON livre na UI. Feature flag local `fashion_enabled` permite ativar por etapa. Migrações são explícitas; backend antigo não deve abrir banco novo sem compatibilidade planejada.
