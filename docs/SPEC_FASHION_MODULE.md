...
Links colados em Markdown e com parâmetros de rastreamento são normalizados antes da captura, preservando parâmetros funcionais como variante. A captura decodifica respostas HTTP compactadas (gzip, deflate e Brotli) com limite de bytes descompactados antes de interpretar a página. A extração tenta dados estruturados de produto e variantes, galerias HTML e metadados sociais; para URLs VTEX do formato `/slug/p`, consulta o catálogo público quando a página não entrega foto e prioriza a variante indicada na URL. No Mercado Livre, pode consultar a API oficial do item quando a página exigir verificação. Se a loja bloquear também essa consulta, a usuária pode manter o link e colar a imagem com Ctrl+V ou anexar uma captura: o chat usa a imagem fornecida e conserva a URL como origem, sem tentar acessar a página bloqueada.
